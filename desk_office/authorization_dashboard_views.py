...
@login_required
@permission_required("desk_office.view")
def authorization_dashboard(request):
    ...
    if request.method == "POST" and "search_patients" in request.POST:
        ...
        if patient_search_form.is_valid():
            ...
            if search_query:
                ...
                patients = (
                    Patient.objects.filter(patient_type="nhia")
                    .filter(
                        ...
                        | Q(
                            retainership_info__retainership_reg_number__icontains=search_query
                        )
    ...
    if request.method == "GET" and "patient_id" in request.GET:
        try:
            selected_patient = Patient.objects.get(
                id=request.GET.get("patient_id"), patient_type="nhia"
            )
            authorization_form = AuthorizationCodeForm(patient=selected_patient)
            messages.info(
                request, f"Selected patient: {selected_patient.get_full_name()}"
            )
        except Patient.DoesNotExist:
            messages.error(
                request, "Selected patient not found or is not an NHIA patient."
            )
    ...
    # Handle authorization code generation
        ...
        try:
            ...
            if is_quick_code:
                ...
                expiry_date = timezone.now().date() + timezone.timedelta(
                    days=expiry_days
                )
                ...
                messages.success(
                    request,
                    f"Quick authorization code {auth_code.code} generated successfully for {selected_patient.get_full_name()}.",
                )
                ...
                authorization_form = AuthorizationCodeForm(
                    request.POST, patient=selected_patient
                )
                ...
                if authorization_form.is_valid():
                    ...
                    messages.success(
                        request,
                        f"Authorization code {auth_code.code} generated successfully for {selected_patient.get_full_name()}.",
                    )
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Selected patient not found or is not an NHIA patient.",
                    }
                )
            ...
            if request.headers.get("X-Requested-With") == "XMLHttpRequest":
                return JsonResponse(
                    {"success": False, "message": f"Invalid amount value: {str(e)}"}
                )
    ...
    pending_radiology = (
        ...
        .select_related("patient", "referring_doctor", "test")
        .order_by("-order_date")
    )
    ...
    pending_surgeries = (
        ...
        .select_related("patient", "surgery_type", "primary_surgeon", "theatre")
    ...
    recent_codes = AuthorizationCode.objects.select_related("patient").order_by(
        "-generated_at"
    )[:10]
    ...
    medical_module_requests = (
        InternalNotification.objects.filter(
            is_read=False, title__icontains="NHIA Authorization Request"
        )
...
@login_required
@permission_required("desk_office.generate_auth_code")
...
def authorization_code_list(request):
    ...
    if request.GET.get("export") == "csv":
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="authorization_codes_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
        ...
        for code in codes:
            nhia_number = (
                code.patient.nhia_info.nhia_reg_number
                if hasattr(code.patient, "nhia_info") and code.patient.nhia_info
                else "N/A"
            )
            writer.writerow(
                [
                    ...
                    code.generated_by.get_full_name()
                    if code.generated_by
                    else "System",
                    code.generated_at.strftime("%Y-%m-%d %H:%M:%S"),
                    code.expiry_date.strftime("%Y-%m-%d")
                    if code.expiry_date
                    else "Never",
                    code.used_at.strftime("%Y-%m-%d %H:%M:%S")
                    if hasattr(code, "used_at") and code.used_at
                    else "Not used",
                    code.notes or "",
                ]
            )
...
def bulk_authorize_consultations(request):
    ...
    if request.method == "POST":
        ...
        if authorized_count > 0:
            messages.success(
                request, f"{authorized_count} consultations have been authorized."
            )
        else:
            messages.warning(
                request, "No NHIA consultations were found among the selected items."
            )
...
def bulk_authorize_referrals(request):
    ...
    if request.method == "POST":
        ...
        if authorized_count > 0:
            messages.success(
                request, f"{authorized_count} referrals have been authorized."
            )
        else:
            messages.warning(
                request, "No NHIA referrals were found among the selected items."
            )
