...
def generate_authorization_code(request):
    ...
    if (
        ...
        and "patient_id" in request.POST
    ):
        try:
    ...
    if request.method == "POST" and "search_patients" in request.POST:
        ...
        if patient_search_form.is_valid():
            ...
            if search_query:
                ...
                patients = Patient.objects.filter(patient_type="nhia").filter(
                ...
                return render(
                    request, "desk_office/generate_authorization_code.html", context
                )
    ...
    if request.method == "GET" and "patient_id" in request.GET:
        try:
            ...
            try:
                if str(patient_identifier).isdigit():
                    selected_patient = Patient.objects.get(
                        id=patient_identifier, patient_type="nhia"
                    )
                else:
                    selected_patient = Patient.objects.get(
                        patient_id=patient_identifier, patient_type="nhia"
                    )
            except Patient.DoesNotExist:
                # Try searching by NHIA number
                selected_patient = Patient.objects.get(
                    nhia_info__nhia_reg_number=patient_identifier, patient_type="nhia"
                )

            # Check for existing active authorization code
            existing_code = AuthorizationCode.objects.filter(
                patient=selected_patient, status="active"
            ).first()
            ...
            if existing_code:
                messages.warning(
                    request,
                    f"This patient already has an active authorization code: {existing_code.code}",
                )
            ...
            authorization_form = AuthorizationCodeForm(
                patient=selected_patient, user=request.user, initial=initial_data
            )
    ...
    if request.method == "POST" and "generate_code" in request.POST:
        try:
            ...
            authorization_form = AuthorizationCodeForm(
                request.POST, patient=selected_patient, user=request.user
            )
            if authorization_form.is_valid():
                authorization_code = authorization_form.save()
                messages.success(
                    request,
                    f"Authorization code {authorization_code.code} generated successfully.",
                )
...
def verify_authorization_code(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request, f"Authorization code {authorization_code.code} is valid."
            )
            return render(
                request,
                "desk_office/verify_authorization_code.html",
...
def search_nhia_patients_ajax(request):
    ...
    if request.method == "GET" and "q" in request.GET:
        ...
        if len(term) >= 2:
            patients = (
                Patient.objects.filter(patient_type=patient_type)
                .filter(
            ...
            for patient in patients:
                ...
                if quick == "true":
                    ...
                    nhia_info = (
                        f'<br><small class="text-info">NHIA: {patient.nhia_info.nhia_reg_number}</small>'
                        if patient.nhia_info
                        else ""
                    )
                    ...
                    """
                    results.append({"id": patient.pk, "html": html})
...
@login_required
@permission_required("desk_office.cancel_auth_code")
    ...
    if request.method == "POST":
        try:
            ...
            if auth_code.status == "active":
                ...
                auth_code.save()
                messages.success(
                    request, f"Authorization code {auth_code.code} has been cancelled."
                )
            else:
                messages.warning(
                    request, "Only active authorization codes can be cancelled."
                )
        except AuthorizationCode.DoesNotExist:
            messages.error(request, "Authorization code not found.")

    return redirect(
        request.META.get("HTTP_REFERER", "desk_office:authorization_code_list")
    )
...
def authorization_code_detail(request, code_id):
    ...
    try:
        ...
        nhia_info = (
            f"<p><strong>NHIA Number:</strong> {auth_code.patient.nhia_info.nhia_reg_number}</p>"
            if hasattr(auth_code.patient, "nhia_info") and auth_code.patient.nhia_info
            else ""
        )
        ...
        if (
            ...
            and hasattr(auth_code, "used_at")
            and auth_code.used_at
        ):
            used_info = f"<p><strong>Used At:</strong> {auth_code.used_at.strftime('%b %d, %Y %H:%M')}</p>"

        # Format notes
        notes_info = (
            f"<p><strong>Notes:</strong> {auth_code.notes}</p>"
            if auth_code.notes
            else ""
        )
        ...
        return HttpResponse(
            '<div class="alert alert-danger"><i class="fas fa-exclamation-triangle"></i> Authorization code not found.</div>'
        )
