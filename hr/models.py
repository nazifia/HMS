...
class StaffSchedule(TenantModel):
    ...
    shift = models.ForeignKey(
        Shift, on_delete=models.CASCADE, related_name="staff_schedules"
    )
...
class Leave(TenantModel):
    ...
    leave_type = models.CharField(
        max_length=20, choices=LEAVE_TYPE_CHOICES, db_index=True
    )
...
class Attendance(TenantModel):
    ...
    def get_working_hours(self):
        ...
        time_out_dt = timezone.datetime.combine(
            timezone.datetime.today(), self.time_out
        )
...
class StaffProfile(TenantModel):
    ...
    department = models.ForeignKey(
        Department, on_delete=models.CASCADE, related_name="staff_profiles"
    )
    designation = models.ForeignKey(
        Designation, on_delete=models.CASCADE, related_name="staff_profiles"
    )
    shift = models.ForeignKey(
        Shift, on_delete=models.SET_NULL, null=True, related_name="staff_profiles"
    )
