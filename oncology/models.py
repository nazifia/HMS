...
class OncologyRecord(TenantModel):
    ...
    requires_authorization = models.BooleanField(
        default=False,
        help_text="True if this NHIA patient oncology record requires desk office authorization"
    )
    ...
    authorization_status = models.CharField(
        ...
        help_text="Status of authorization for this oncology record"
    )
    authorization_code = models.ForeignKey('nhia.AuthorizationCode', on_delete=models.SET_NULL, null=True, blank=True, related_name='oncology_records')
...
class OncologyClinicalNote(NigerianClerkingNote, TenantModel):
    """Nigerian clerking proforma clinical notes for oncology records"""
    oncology_record = models.ForeignKey(OncologyRecord, on_delete=models.CASCADE, related_name='clinical_notes')
    created_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name='oncology_clinical_notes_created')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"Clinical Note for {self.oncology_record.patient.get_full_name()} - {self.created_at.strftime('%Y-%m-%d %H:%M')}"

    class Meta:
        ordering = ['-created_at']
        verbose_name = "Oncology Clinical Note"
        verbose_name_plural = "Oncology Clinical Notes"
