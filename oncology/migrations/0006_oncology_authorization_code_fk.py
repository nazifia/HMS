...
Oncology stored the desk office's code as a CharField, so nothing checked it
against `nhia.AuthorizationCode` — a typo, an expired code or an invented one
all looked the same. Existing text is matched to a real code where one exists
and dropped where it does not, which is the honest outcome: an unmatched string
was never an authorization.
"""
...
def link_existing_codes(apps, schema_editor):
    OncologyRecord = apps.get_model("oncology", "OncologyRecord")
    AuthorizationCode = apps.get_model("nhia", "AuthorizationCode")

    for record in OncologyRecord.objects.exclude(
...
def unlink(apps, schema_editor):
    OncologyRecord = apps.get_model("oncology", "OncologyRecord")
    for record in OncologyRecord.objects.filter(
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ("oncology", "0005_oncologyclinicalnote_hospital_and_more"),
    ]

    operations = [
        migrations.RenameField(
            model_name="oncologyrecord",
