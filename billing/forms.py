...
class InvoiceForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "patient": forms.Select(
                attrs={"class": "form-select select2 patient-select"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk and not self.initial.get("due_date"):
            self.initial["due_date"] = timezone.now().date() + timezone.timedelta(
                days=30
            )
    ...
    def clean(self):
        ...
        if not service and not description:
            raise forms.ValidationError(
                "Either select a service or provide a description."
            )
