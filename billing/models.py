...
class Invoice(TenantModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id or hasattr(
            self, "subtotal"
        ):  # Ensure subtotal exists if we are to calculate total
            self.total_amount = (
                (self.subtotal or 0)
        ...
        if self.total_amount > 0:
            if self.amount_paid >= self.total_amount:
                ...
                self.status = (
                    "pending"  # Or back to pending if payment was reversed for e.g.
                )
            ...
            if hasattr(self, "_auto_pay_zero_amount") and self._auto_pay_zero_amount:
                ...
                self.status = (
                    "pending"  # Keep zero amount invoices as pending by default
                )
        ...
        if self.status == "paid" and self.prescription:
            try:
                ...
                should_update = (
                    hasattr(self, "_manual_payment_processed")
                    or self.total_amount > 0
                    or (
                        hasattr(self, "_auto_pay_zero_amount")
                        and self._auto_pay_zero_amount
                    )
                )
    ...
    def _generate_invoice_number(self):
        ...
        timestamp = str(int(time.time() * 1000))[
            -4:
        ]  # Last 4 digits of millisecond timestamp
...
class InvoiceItem(TenantModel):
    ...
    def save(self, *args, **kwargs):
        ...
        self.total_amount = (
            (self.unit_price * self.quantity) + self.tax_amount - self.discount_amount
        )
