...
from .forms import DermatologyRecordForm, DermatologyRecordSearchForm, DermatologyServiceForm, DermatologyTestForm, DermatologyClinicalNoteForm
...
@login_required
@department_access_required('Dermatology')
def dermatology_dashboard(request):
    """Enhanced Dashboard for Dermatology department with charts and metrics"""
    ...
    context = build_enhanced_dashboard_context(
        ...
        record_model=DermatologyRecord,
        record_queryset=DermatologyRecord.objects.all(),
        priority_field=None,  # Dermatology records don't have priority field
        status_field='treatment_status',
        completed_status='completed'
    )

    # Add dermatology-specific statistics
    today = timezone.now().date()

    # Treatment status statistics
    planned_treatments = DermatologyRecord.objects.filter(treatment_status='planned').count()
    in_progress_treatments = DermatologyRecord.objects.filter(treatment_status='in_progress').count()
    completed_treatments = DermatologyRecord.objects.filter(treatment_status='completed').count()
    completed_today = DermatologyRecord.objects.filter(
        treatment_status='completed',
        updated_at__date=today
    ).count()

    # Appointments today
    appointments_today = DermatologyRecord.objects.filter(
        appointment_date__date=today
    ).count()

    # Follow-ups due this week
    week_end = today + timedelta(days=7)
    followups_due = DermatologyRecord.objects.filter(
        next_appointment_date__date__gte=today,
        next_appointment_date__date__lte=week_end
    ).count()

    # Common conditions (top 5)
    condition_data = DermatologyRecord.objects.filter(
        condition_type__isnull=False
    ).exclude(condition_type='').values('condition_type').annotate(count=Count('id')).order_by('-count')[:5]
    condition_labels = [dict(DermatologyRecord.CONDITION_TYPE_CHOICES).get(item['condition_type'], item['condition_type']) for item in condition_data]  # type: ignore
    condition_counts = [item['count'] for item in condition_data]

    # Emergency dermatology cases (records created today with urgent notes)
    emergency_cases = DermatologyRecord.objects.filter(
        created_at__date=today,
        diagnosis__icontains='emergency'
    ).count() + DermatologyRecord.objects.filter(
        created_at__date=today,
    ...
    # Add to context
    context.update({
        'total_records': DermatologyRecord.objects.count(),
        'records_today': DermatologyRecord.objects.filter(created_at__date=today).count(),
        'records_this_week': DermatologyRecord.objects.filter(created_at__date__gte=today - timedelta(days=7)).count(),
        ...
        'condition_labels': json_for_template(condition_labels),
        'condition_counts': json_for_template(condition_counts),
    ...
    return render(request, 'dermatology/dashboard.html', context)


@login_required
def dermatology_records(request):
    """View to list all dermatology records with search and pagination"""
    records = DermatologyRecord.objects.select_related('patient', 'service', 'dermatologist').all()
    
    # Add search functionality
    search_form = DermatologyRecordSearchForm(request.GET)
    ...
    return render(request, 'dermatology/dermatology_records.html', context)

@login_required
def create_dermatology_record(request):
    """View to create a new dermatology record"""
    if request.method == 'POST':
        form = DermatologyRecordForm(request.POST)
        if form.is_valid():
            ...
            if patient:
                ...
                if user_department:
                    ...
                    if unauthorized_referrals.exists():
                        ...
                        messages.error(
                            request,
                            f"Cannot create dermatology record for {patient.get_full_name()}. "
...
@login_required
def dermatology_record_detail(request, record_id):
    """View to display details of a specific dermatology record"""
    record = get_object_or_404(DermatologyRecord.objects.select_related('patient', 'service', 'dermatologist'), id=record_id)
    ...
    tests = DermatologyTest.objects.filter(dermatology_record=record).order_by('-performed_at')

    # Get clinical notes for this record
    clinical_notes = DermatologyClinicalNote.objects.filter(dermatology_record=record).select_related('created_by').order_by('-created_at')
    ...
    if is_nhia_patient and requires_authorization:
        ...
        has_pending_request = InternalNotification.objects.filter(
            ...
            Q(message__icontains="Module: dermatology") &
            Q(is_read=False)
        ).exists()
    ...
    if is_nhia_patient:
        if record.authorization_code:
            ...
            messages.warning(
                ...
                f"Please contact the desk office to obtain authorization for dermatology services."
            )
    ...
    return render(request, 'dermatology/dermatology_record_detail.html', context)

@login_required
def edit_dermatology_record(request, record_id):
    """View to edit an existing dermatology record"""
    record = get_object_or_404(DermatologyRecord, id=record_id)

    if request.method == 'POST':
        form = DermatologyRecordForm(request.POST, instance=record)
        if form.is_valid():
            record = form.save()
            messages.success(request, 'Dermatology record updated successfully.')
            return redirect('dermatology:dermatology_record_detail', record_id=record.id)
    else:
        form = DermatologyRecordForm(instance=record)
...
@login_required
def delete_dermatology_record(request, record_id):
    """View to delete a dermatology record"""
    record = get_object_or_404(DermatologyRecord, id=record_id)
    
    if request.method == 'POST':
        record.delete()
        messages.success(request, 'Dermatology record deleted successfully.')
        return redirect('dermatology:dermatology_records')
    
    context = {
        'record': record
    }
    return render(request, 'dermatology/dermatology_record_confirm_delete.html', context)

@login_required
def search_dermatology_patients(request):
    """AJAX view for searching patients in dermatology module"""
...
@login_required
def create_prescription_for_dermatology(request, record_id):
    """Create a prescription for a dermatology patient"""
    record = get_object_or_404(DermatologyRecord, id=record_id)
        ...
        if prescription_form.is_valid() and formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    return redirect('dermatology:dermatology_record_detail', record_id=record.pk)
    ...
    return render(request, 'dermatology/create_prescription.html', context)

@login_required
def dermatology_services(request):
    """View to list and manage dermatology services"""
    services = DermatologyService.objects.all().order_by('name')
    
    if request.method == 'POST':
        form = DermatologyServiceForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, 'Dermatology service created successfully.')
            return redirect('dermatology:dermatology_services')
    else:
        form = DermatologyServiceForm()
...
@login_required
def delete_dermatology_service(request, service_id):
    """View to delete a dermatology service"""
    service = get_object_or_404(DermatologyService, id=service_id)
    
    if request.method == 'POST':
        service.delete()
        messages.success(request, 'Dermatology service deleted successfully.')
        return redirect('dermatology:dermatology_services')
    
    context = {
        'service': service
    }
    return render(request, 'dermatology/dermatology_service_confirm_delete.html', context)

@login_required
def add_test_to_dermatology_record(request, record_id):
    """Add a test to a dermatology record"""
    record = get_object_or_404(DermatologyRecord, id=record_id)
    
    if request.method == 'POST':
        form = DermatologyTestForm(request.POST)
        if form.is_valid():
            test = form.save(commit=False)
            test.dermatology_record = record
            test.performed_by = request.user
            test.save()
            messages.success(request, 'Test added successfully.')
            return redirect('dermatology:dermatology_record_detail', record_id=record.pk)
    else:
        form = DermatologyTestForm()
    ...
    return render(request, 'dermatology/add_test.html', context)

@login_required
def delete_test(request, test_id):
    """Delete a dermatology test"""
    test = get_object_or_404(DermatologyTest, id=test_id)
    record_id = test.dermatology_record.pk
    
    if request.method == 'POST':
        test.delete()
        messages.success(request, 'Test deleted successfully.')
        return redirect('dermatology:dermatology_record_detail', record_id=record_id)
    
    context = {
        'test': test
    }
    return render(request, 'dermatology/test_confirm_delete.html', context)

@login_required
def generate_invoice_for_dermatology(request, record_id):
    """Generate an invoice for a dermatology record"""
    record = get_object_or_404(DermatologyRecord, id=record_id)

    # Check if invoice already exists
    if record.invoice:
        messages.info(request, 'An invoice already exists for this dermatology record.')
        return redirect('dermatology:dermatology_record_detail', record_id=record.pk)
    ...
    if record.patient.is_nhia_patient():
        ...
        if not record.authorization_code.is_valid():
            messages.error(
                request,
                f"Cannot generate invoice. The authorization code for this dermatology record is expired or invalid. "
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                tax_amount = Decimal('0.00')  # No tax for dermatology services
                ...
                invoice = Invoice.objects.create(
                    ...
                    due_date=due_date,
                    status='pending',
                    source_app='other'  # Dermatology doesn't have a specific source_app choice
                )

                # Link invoice to dermatology record
    ...
    return render(request, 'dermatology/generate_invoice.html', context)
...
def add_clinical_note(request, record_id):
    """Add a clinical note (SOAP format) to a dermatology record"""
    record = get_object_or_404(DermatologyRecord, id=record_id)

    if request.method == 'POST':
        form = DermatologyClinicalNoteForm(request.POST)
        if form.is_valid():
            clinical_note = form.save(commit=False)
            clinical_note.dermatology_record = record
            clinical_note.created_by = request.user
            clinical_note.save()
            messages.success(request, 'Clinical note added successfully.')
            return redirect('dermatology:dermatology_record_detail', record_id=record.pk)
    else:
        form = DermatologyClinicalNoteForm()
    ...
    return render(request, 'dermatology/clinical_note_form.html', context)


@login_required
def edit_clinical_note(request, note_id):
    """Edit an existing clinical note"""
    note = get_object_or_404(DermatologyClinicalNote, id=note_id)
    record = note.dermatology_record

    if request.method == 'POST':
        form = DermatologyClinicalNoteForm(request.POST, instance=note)
        if form.is_valid():
            form.save()
            messages.success(request, 'Clinical note updated successfully.')
            return redirect('dermatology:dermatology_record_detail', record_id=record.pk)
    else:
        form = DermatologyClinicalNoteForm(instance=note)
...
def delete_clinical_note(request, note_id):
    ...
    note = get_object_or_404(DermatologyClinicalNote, id=note_id)
    record_id = note.dermatology_record.pk
    ...
    return render(request, 'dermatology/clinical_note_confirm_delete.html', context)
...
def view_clinical_note(request, note_id):
    ...
    return render(request, 'dermatology/clinical_note_detail.html', context)
