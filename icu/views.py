...
from .forms import IcuRecordForm, IcuRecordSearchForm, IcuClinicalNoteForm
...
def add_clinical_note(request, record_id):
    """Add a clinical note (SOAP format) to a icu record"""
    record = get_object_or_404(IcuRecord, id=record_id)

    if request.method == 'POST':
        form = IcuClinicalNoteForm(request.POST)
        if form.is_valid():
            clinical_note = form.save(commit=False)
            clinical_note.icu_record = record
            clinical_note.created_by = request.user
            clinical_note.save()
            messages.success(request, 'Clinical note added successfully.')
            return redirect('icu:icu_record_detail', record_id=record.pk)
    else:
        form = IcuClinicalNoteForm()
    ...
    return render(request, 'icu/clinical_note_form.html', context)


@login_required
def edit_clinical_note(request, note_id):
    """Edit an existing clinical note"""
    note = get_object_or_404(IcuClinicalNote, id=note_id)
    record = note.icu_record

    if request.method == 'POST':
        form = IcuClinicalNoteForm(request.POST, instance=note)
        if form.is_valid():
            form.save()
            messages.success(request, 'Clinical note updated successfully.')
            return redirect('icu:icu_record_detail', record_id=record.pk)
    else:
        form = IcuClinicalNoteForm(instance=note)
...
def delete_clinical_note(request, note_id):
    ...
    note = get_object_or_404(IcuClinicalNote, id=note_id)
    record_id = note.icu_record.pk

    if request.method == 'POST':
        note.delete()
        messages.success(request, 'Clinical note deleted successfully.')
        return redirect('icu:icu_record_detail', record_id=record_id)

    context = {
        'note': note
    }
    return render(request, 'icu/clinical_note_confirm_delete.html', context)
...
def view_clinical_note(request, note_id):
    ...
    return render(request, 'icu/clinical_note_detail.html', context)
