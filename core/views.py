...
def notifications_list(request):
    ...
    """
    notifications = InternalNotification.objects.filter(user=request.user).order_by(
        "-created_at"
    )
...
def mark_notification_read(request, notification_id):
    ...
    if request.user.is_staff or request.user.is_superuser:
        ...
        notification = get_object_or_404(
            InternalNotification, id=notification_id, user=request.user
        )
...
def create_prescription_view(request, patient_id, module_name):
    ...
    messages.info(
        request, "Prescription creation functionality would be implemented here."
    )
    return redirect("dashboard:dashboard")
...
def patient_prescriptions_view(request, patient_id):
    ...
    messages.info(
        request, "Patient prescriptions functionality would be implemented here."
    )
    return redirect("dashboard:dashboard")
...
def search_patients(request):
    """
    Search patients API endpoint - returns JSON with patient data
    """
        ...
        patient_data.append(
            {
                ...
                "age": patient.get_age() if hasattr(patient, "get_age") else "N/A",
...
def patient_search_ajax(request):
    ...
    patient_type = request.GET.get(
        "patient_type", ""
    )  # Optional filter for NHIA/non-NHIA

    if not query or len(query) < 2:
        return render(
            request,
            "pharmacy/pack_orders/partials/patient_search_results.html",
...
def get_authorization_codes_for_patient(request):
    ...
    if not patient_id:
        return render(
            request,
            "pharmacy/pack_orders/partials/authorization_codes.html",
            {"authorization_codes": [], "is_nhia": False, "patient_id": None},
        )
    ...
    try:
        ...
        return render(
            request,
            "pharmacy/pack_orders/partials/authorization_codes.html",
            {"authorization_codes": [], "is_nhia": False, "patient_id": patient_id},
        )
...
def request_nhia_authorization_form(request, model_type, object_id):
    ...
    if request.method == "POST":
        ...
        if not desk_office_users.exists():
            messages.error(
                request,
                "No desk office staff available to receive authorization requests.",
            )
            return render(
                request,
                "core/request_authorization_form.html",
                {
                    "record": record,
...
Please generate an authorization code for this patient to proceed with treatment and billing.
        ...
        messages.success(
            request, "Authorization request sent to desk office successfully."
        )
        return redirect(request.META.get("HTTP_REFERER", "dashboard:dashboard"))
...
def request_nhia_authorization(request):
    ...
    try:
        ...
        existing_notification = InternalNotification.objects.filter(
            message__contains=f"Record ID: {record_id}", is_read=False
        ).exists()
        ...
        existing_notification = InternalNotification.objects.filter(
            Q(
                message__contains=f"Patient: {patient.get_full_name()} (ID: {patient.patient_id})"
            )
