...
def get_role_badge_class(role_name):
    ...
    role_badges = {
        ...
        "desk_officer": "bg-dark",
        "cashier_accountant": "bg-success text-white",
    }
...
def is_admin_user(context):
    ...
    return user.is_authenticated and (
        user.is_superuser
        or (hasattr(user, "profile") and user.profile and user.profile.role == "admin")
    )
...
def can_view_sensitive_data(context):
    ...
    user_roles = (
        [role.name for role in user.roles.all()] if hasattr(user, "roles") else []
    )
