...
def calculate_test_request_estimated_cost(test_request):
    ...
    return (
        total_cost if total_cost > 0 else 5000.00
    )  # Default minimum cost if no tests found
...
def generate_authorization(request, model_type, object_id):
    ...
    if obj.patient.patient_type != "nhia":
        messages.error(request, "Authorization is only required for NHIA patients.")
        return redirect(
            request.META.get("HTTP_REFERER", "desk_office:authorization_dashboard")
        )
    ...
    if request.method == "POST":
        ...
        if code_type == "manual":
            ...
            if auth_code:
                ...
                messages.error(
                    request, f"Failed to generate authorization code: {error}"
                )
...
def universal_authorization_dashboard(request):
    ...
    if search_query:
        ...
        for model_type, data in pending_items.items():
            filtered_items = [
                ...
                if search_query.lower() in item.patient.get_full_name().lower()
                or search_query.lower() in str(item.patient.patient_id).lower()
            ]
            if filtered_items:
    ...
    recent_codes_query = AuthorizationCode.objects.select_related(
    ...
    stats = {
        ...
        "today_generated": AuthorizationCode.objects.filter(
...
def authorization_history(request, model_type, object_id):
    ...
    auth_codes = AuthorizationCode.objects.filter(
        patient=obj.patient, service_type=model_info["service_type"]
