...
def add_purchase_item(request, purchase_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Update purchase total using model method
...
def dispense_prescription(request, prescription_id):
    ...
    Read-only: shows the NHIA/payment breakdown and routes to the cart flow
    (create_cart_from_prescription), which is where dispensing now happens.
    The legacy per-item formset this view used to build was never rendered by
    the template, so it has been removed.
    """
    prescription = get_object_or_404(
        Prescription.objects.select_related("patient", "doctor", "doctor__profile"),
        id=prescription_id,
    )

    # Authorization / state check before showing the dispense page
    ...
    context = {
        ...
        "total_patient_pays": pricing_breakdown["patient_portion"],
        "total_nhia_covers": pricing_breakdown["nhia_portion"],
    }
