...
class PrescriptionCart(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        (
            "partially_dispensed",
            "Partially Dispensed",
        ),  # Some items dispensed, others pending
        ("completed", "Completed"),  # All items fully dispensed
        ("cancelled", "Cancelled"),  # Cart cancelled
    )

    prescription = models.ForeignKey(
        "pharmacy.Prescription", on_delete=models.CASCADE, related_name="carts"
    )
    ...
    def can_generate_invoice(self):
        ...
        for item in self.items.all():
            if not item.has_sufficient_stock():
                return (
                    False,
                    f"Insufficient stock for {item.prescription_item.medication.name}",
                )
    ...
    def can_complete_dispensing(self):
        ...
        has_items_to_dispense = any(
            item.get_remaining_quantity() > 0 for item in self.items.all()
        )
    ...
    def get_dispensing_progress(self):
        ...
        fully_dispensed = sum(
            1 for item in self.items.all() if item.is_fully_dispensed()
        )
        partially_dispensed = sum(
            1 for item in self.items.all() if item.is_partially_dispensed()
        )
        ...
        percentage = (
            int((dispensed_quantity / total_quantity) * 100)
            if total_quantity > 0
            else 0
        )
...
class PrescriptionCartItem(models.Model):
    ...
    cart = models.ForeignKey(
        PrescriptionCart, on_delete=models.CASCADE, related_name="items"
    )

    prescription_item = models.ForeignKey(
        "pharmacy.PrescriptionItem", on_delete=models.CASCADE, related_name="cart_items"
    )

    quantity = models.IntegerField(help_text="Total quantity to dispense/bill")

    quantity_dispensed = models.IntegerField(
        default=0, help_text="Quantity already dispensed from this cart item"
    )
    ...
    available_stock = models.IntegerField(
        default=0, help_text="Available stock at time of adding to cart (cached)"
    )
    ...
    substitute_reason = models.TextField(
        blank=True, null=True, help_text="Reason for medication substitution"
    )
    ...
    substituted_at = models.DateTimeField(
        null=True, blank=True, help_text="When the substitution was made"
    )
    ...
    def get_effective_medication(self):
        ...
        return (
            self.substitute_medication
            if self.substitute_medication
            else self.prescription_item.medication
        )
    ...
    def substitute_with(self, new_medication, reason, user):
        ...
        if new_medication.id == self.prescription_item.medication.id:
            raise ValidationError(
                "Substitute medication cannot be the same as prescribed medication"
            )
        ...
        try:
            ...
            if hasattr(dispensary, "active_store"):
                try:
                    ...
                    logger.warning(
                        f"Error accessing active_store for dispensary {dispensary.id}: {e}"
                    )
        except Exception as e:
