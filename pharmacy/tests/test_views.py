...
class PharmacyViewsTest(TestCase):
    def setUp(self):
        ...
        view_perm = Permission.objects.get(
            codename="view_prescription", content_type=prescription_ct
        )
        ...
        self.dispensary = Dispensary.objects.create(
            name="Test Dispensary", location="Main Hospital", is_active=True
        )
    ...
    def test_medication_detail_view(self):
        ...
        response = self.client.get(
            reverse("pharmacy:medication_detail", args=[self.medication.id])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_prescription_detail_view(self):
        ...
        response = self.client.get(
            reverse("pharmacy:prescription_detail", args=[self.prescription.id])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_medication_autocomplete_view(self):
        ...
        response = self.client.get(
            reverse("pharmacy:medication_autocomplete"), {"term": "Ibu"}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
