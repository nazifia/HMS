...
class ComprehensiveRevenueFilterForm(forms.Form):
    """Enhanced form for filtering comprehensive revenue analysis with month-based options"""
    ...
    YEAR_CHOICES = [(y, y) for y in range(2020, timezone.now().year + 3)]
    ...
    # Month/Year selection for specific month filtering
    ...
    departments = forms.MultipleChoiceField(
        ...
        help_text='Select departments to include in analysis (leave empty for all)'
    )
    
    # Export options
    include_daily_breakdown = forms.BooleanField(
        required=False,
        initial=False,
        widget=forms.CheckboxInput(attrs={
            'class': 'form-check-input'
        }),
        label='Include Daily Breakdown',
        help_text='Include day-by-day revenue breakdown in analysis'
    )
    ...
    def clean(self):
        ...
        # Validate specific month selection
        if filter_type == 'specific_month':
            if not month or not year:
                raise ValidationError('Month and year are required for specific month selection.')
    ...
    def get_date_range(self):
        """Get the actual date range based on filter type"""
        from .revenue_service import MonthFilterHelper
        ...
        if filter_type == 'current_month':
            ...
            return MonthFilterHelper.get_previous_month()
        elif filter_type == 'last_3_months':
            return MonthFilterHelper.get_last_n_months(3)
        elif filter_type == 'last_6_months':
            return MonthFilterHelper.get_last_n_months(6)
        elif filter_type == 'last_12_months':
            return MonthFilterHelper.get_last_n_months(12)
        elif filter_type == 'year_to_date':
            return MonthFilterHelper.get_year_to_date()
        elif filter_type == 'specific_month':
            month = int(self.cleaned_data.get('month'))
            year = int(self.cleaned_data.get('year'))
            return MonthFilterHelper.get_specific_month(year, month)
