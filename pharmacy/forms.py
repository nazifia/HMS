...
class PurchaseForm(forms.ModelForm):
    ...
    def clean_invoice_number(self):
        ...
        if (
            invoice_number
            and Purchase.objects.filter(invoice_number=invoice_number).exists()
        ):
...
class PurchaseItemForm(forms.ModelForm):
    ...
    class Meta:
        model = PurchaseItem
        fields = ["medication", "quantity", "unit_price", "batch_number", "expiry_date"]
...
class PrescriptionForm(forms.ModelForm):
    ...
    def _format_patient_label(self, obj):
        ...
        if hasattr(obj, "nhia_info") and obj.nhia_info and obj.nhia_info.is_active:
            ...
            hasattr(obj, "retainership_info")
            and obj.retainership_info
            and obj.retainership_info.is_active
        ):
    ...
    diagnosis = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Enter diagnosis (optional)"}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if patient_instance:
            ...
            patient_id_for_filter = (
                patient_instance.id
                if hasattr(patient_instance, "id")
                else patient_instance
            )
            self.fields["patient"].queryset = Patient.objects.filter(
                id=patient_id_for_filter
            )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "notes": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Enter additional notes (optional)"}
            ),
...
class DispenseItemForm(forms.Form):
    ...
    dispensary = forms.ModelChoiceField(
        ...
        widget=forms.Select(
            attrs={"class": "form-control form-control-sm d-none"}
        ),  # Hidden since we have global selection
    )
    stock_quantity_display = forms.CharField(
        required=False,
        widget=forms.TextInput(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.prescription_item:
            logging.debug(
                f"DispenseItemForm __init__: Initializing for item ID {self.prescription_item.id}"
            )
            logging.debug(f"  Medication: {self.prescription_item.medication.name}")
            logging.debug(
                f"  Remaining Qty: {self.prescription_item.remaining_quantity_to_dispense}"
            )
            ...
            if is_fully_dispensed:
                ...
                self.fields["dispense_this_item"].widget.attrs["title"] = (
                ...
                self.fields["quantity_to_dispense"].widget.attrs["placeholder"] = (
                    "Enter quantity"
                )
            ...
            if self.selected_dispensary:
                if available_stock > 0:
                    ...
                    ].initial = f"{available_stock} in stock"
                else:
    ...
    def p_item(self):
        ...
        logging.debug(
            f"Accessing p_item property. Item ID: {self.prescription_item.id if self.prescription_item else 'None'}"
        )
    ...
    def clean(self):
        ...
        logging.debug(
            f"DispenseItemForm clean: Item ID {self.prescription_item.id if self.prescription_item else 'None'}"
        )
        logging.debug(
            f"  dispense_this_item: {dispense_this_item}, quantity_to_dispense: {quantity_to_dispense} (type: {type(quantity_to_dispense)}), dispensary: {dispensary}"
        )
        ...
        if not self.prescription_item:
            ...
            logging.error(
                "DispenseItemForm clean: Form not initialized with a prescription item."
            )
            return cleaned_data
        ...
        logging.debug(
            f"  Remaining Qty: {remaining_qty}, Is Fully Dispensed: {is_fully_dispensed}, Available Stock: {available_stock}"
        )
        print(
            f"DEBUG: DispenseItemForm clean - Item ID: {self.prescription_item.id if self.prescription_item else 'None'}"
        )
        ...
        if available_stock is None:
            ...
            can_be_dispensed = (
                remaining_qty > 0 and available_stock > 0 and not is_fully_dispensed
            )
        ...
        if dispense_this_item and not can_be_dispensed:
            ...
            if is_fully_dispensed:
                ...
                error_message = (
                    "Please select a dispensary to check stock for this item."
                )
        ...
        if dispense_this_item:
            ...
            if not effective_dispensary:
                self.add_error(
                    "dispensary", "Please select a dispensary for this item."
                )
                logging.warning(
                    f"  Validation Error: Dispensary not selected for item {self.prescription_item.id}."
                )
            ...
            if quantity_to_dispense is None or quantity_to_dispense == "":
                self.add_error(
                    ...
                    "Quantity is required when item is selected for dispensing.",
                )
                logging.warning(
                    f"  Validation Error: Quantity is missing for item {self.prescription_item.id}."
                )
            elif quantity_to_dispense <= 0:
                self.add_error(
                    "quantity_to_dispense",
                    "Quantity must be greater than 0 if selected for dispensing.",
                )
                logging.warning(
                    f"  Validation Error: Invalid quantity ({quantity_to_dispense}) for item {self.prescription_item.id}."
                )
            elif available_stock is not None and quantity_to_dispense > available_stock:
                self.add_error(
                    "quantity_to_dispense",
                    f"Not enough stock at selected dispensary. Available: {available_stock}.",
                )
                logging.warning(
                    f"  Validation Error: Quantity ({quantity_to_dispense}) > available stock ({available_stock}) for item {self.prescription_item.id}."
                )
...
class BaseDispenseItemFormSet(BaseFormSet):
    ...
    def add_fields(self, form, index):
        ...
        if self.prescription_items_qs and index < len(self.prescription_items_qs):
            ...
            form.prescription_item = (
                None  # Or raise an error, depending on desired behavior
            )
    ...
    def clean(self):
        ...
        for form in self.forms:
            ...
            if (
                ...
                and form.cleaned_data.get("dispense_this_item")
            ):
                any_selected = True
                break
        ...
        if any_selected:
            ...
            global_selected_dispensary = (
                self.custom_form_kwargs.get("selected_dispensary")
                if self.custom_form_kwargs
                else None
            )
            ...
            if not all_have_dispensary:
                raise forms.ValidationError(
                    "Please select a dispensary for all items to be dispensed."
                )
...
class PrescriptionSearchForm(forms.Form):
    ...
    patient_number = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"placeholder": "Enter patient ID/number", "class": "form-control"}
        ),
    ...
    medication_name = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"placeholder": "Search by medication name", "class": "form-control"}
        ),
...
class DispensedItemsSearchForm(forms.Form):
    ...
    patient_name = forms.CharField(
        ...
        max_length=100,
        widget=forms.TextInput(
            attrs={"placeholder": "Patient name...", "class": "form-control"}
        ),
    ...
    min_quantity = forms.IntegerField(
        ...
        min_value=1,
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": "Min quantity"}
        ),
    ...
    max_quantity = forms.IntegerField(
        ...
        min_value=1,
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": "Max quantity"}
        ),
    ...
    class Meta:
        ...
        widgets = {
            "name": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Enter dispensary name"}
            ),
...
class PrescriptionPaymentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.patient_wallet:
            ...
            self.fields[
                "payment_source"
            ].help_text = f"Wallet Balance: ₦{wallet_balance:.2f}"
...
class DispensarySearchForm(forms.Form):
    ...
    location = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Filter by location..."}
        ),
        label="Location",
    )
    ...
    def clean(self):
        ...
        if (
            created_date_from
            and created_date_to
            and created_date_from > created_date_to
        ):
            raise forms.ValidationError("Start date cannot be after end date.")
...
class MedicalPackForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "pack_type": forms.Select(
                attrs={"class": "form-select", "onchange": "handlePackTypeChange()"}
            ),
            "surgery_type": forms.Select(
                attrs={"class": "form-select", "id": "surgery-type-select"}
            ),
            "labor_type": forms.Select(
                attrs={"class": "form-select", "id": "labor-type-select"}
            ),
...
class PackItemForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "is_optional"
        ].help_text = "Optional items can be omitted if unavailable"
        self.fields[
            "order"
        ].help_text = "Order of usage in procedure (0 for no specific order)"
...
class PackOrderForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.preselected_patient:
            ...
            self.fields[
                "patient"
            ].required = False  # Make it not required since we use patient_hidden
            self.fields["patient"].widget.attrs.update(
                {
                    "readonly": True,
                    "disabled": True,
            ...
            self.fields["patient"].queryset = Patient.objects.filter(
                id=self.preselected_patient.id
            )
        ...
        if self.surgery:
            ...
            if surgery_type:
                self.fields["pack"].queryset = MedicalPack.objects.filter(
                    is_active=True, pack_type="surgery", surgery_type=surgery_type
                )
            else:
                self.fields["pack"].queryset = MedicalPack.objects.filter(
                    is_active=True, pack_type="surgery"
                )

        elif self.labor_record:
            # Filter to labor packs
            self.fields["pack"].queryset = MedicalPack.objects.filter(
                is_active=True, pack_type="labor"
            )
        ...
        try:
            ...
            self.fields[
                "authorization_code"
            ].queryset = AuthorizationCode.objects.filter(status="active").order_by(
                "-generated_at"
            )

            # Make authorization code required for NHIA patients if patient is known
            patient = (
                self.preselected_patient or self.instance.patient
                if self.instance.pk
                else None
            )
            if patient:
                ...
                if is_nhia:
                    ...
                    self.fields[
                        "authorization_code"
                    ].label = "Authorization Code (Required for NHIA)"
        except ImportError:
...
class PackFilterForm(forms.Form):
    ...
    search = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Search packs..."}
        ),
        label="Search",
    )
