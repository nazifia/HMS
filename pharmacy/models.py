...
class Purchase(models.Model):
    ...
    def calculate_total_amount(self):
        ...
        total = (
            self.items.aggregate(total=Sum(F("quantity") * F("unit_price")))["total"]
            or 0
        )
        return total
...
class PurchasePayment(models.Model):
    ...
    received_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="purchase_payments_received",
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class PurchaseItem(models.Model):
    ...
    def _add_to_bulk_store(self):
        ...
        bulk_inventory, created = BulkStoreInventory.objects.get_or_create(
            ...
            or f"BATCH-{timezone.now().strftime('%Y%m%d')}-{self.pk or 'NEW'}",
            defaults={
                "stock_quantity": 0,
...
class PharmacistDispensaryAssignment(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="Any additional notes about this assignment"
    )
...
class Dispensary(models.Model):
    ...
    manager = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="managed_dispensaries",
    )
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def assign_pharmacist(self, pharmacist, start_date=None):
        ...
        PharmacistDispensaryAssignment.objects.create(
            pharmacist=pharmacist, dispensary=self, start_date=start_date
        )
    ...
    def remove_pharmacist(self, pharmacist):
        ...
        assignment = PharmacistDispensaryAssignment.objects.filter(
            pharmacist=pharmacist, dispensary=self, end_date__isnull=True
        ).first()
...
class ActiveStore(models.Model):
    ...
    dispensary = models.OneToOneField(
        Dispensary, on_delete=models.CASCADE, related_name="active_store"
    )
    name = models.CharField(max_length=100)
    location = models.CharField(max_length=200, blank=True, null=True)
    description = models.TextField(blank=True, null=True)
...
class BulkStoreInventory(models.Model):
    ...
    medication = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="bulk_inventories"
    )
    bulk_store = models.ForeignKey(
        BulkStore, on_delete=models.CASCADE, related_name="inventories"
    )
    ...
    marked_up_cost = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0.00,
...
class ActiveStoreInventory(models.Model):
    ...
    medication = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="active_inventories"
    )
    active_store = models.ForeignKey(
        ActiveStore, on_delete=models.CASCADE, related_name="inventories"
    )
    ...
    stock_quantity = models.IntegerField(
        default=0, help_text="Total quantity across all batches"
    )
    reorder_level = models.IntegerField(
        default=10, help_text="Minimum stock level before reorder is needed"
    )
    expiry_date = models.DateField(
        blank=True, null=True, help_text="Earliest expiry date (from FIFO)"
    )
    unit_cost = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, help_text="Weighted average cost"
    )
    last_restock_date = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def update_summary_fields(self):
        ...
        self.stock_quantity = (
            batches.aggregate(total=models.Sum("quantity"))["total"] or 0
        )
...
class ActiveStoreBatch(models.Model):
    ...
    active_inventory = models.ForeignKey(
        ActiveStoreInventory, on_delete=models.CASCADE, related_name="batches"
    )
    batch_number = models.CharField(max_length=50)
    ...
    class Meta:
        ordering = [
            "expiry_date",
            "received_date",
        ]  # FIFO: oldest expiry first, then oldest received
...
class MedicationTransfer(models.Model):
    ...
    medication = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="transfers"
    )
    from_bulk_store = models.ForeignKey(
        BulkStore, on_delete=models.CASCADE, related_name="outgoing_transfers"
    )
    to_active_store = models.ForeignKey(
        ActiveStore, on_delete=models.CASCADE, related_name="incoming_transfers"
    )
    quantity = models.IntegerField()
    batch_number = models.CharField(max_length=50, blank=True, null=True)
    expiry_date = models.DateField(blank=True, null=True)
    ...
    delivered_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="delivered_transfers",
    )
...
class DispensingLog(models.Model):
    ...
    prescription_item = models.ForeignKey(
        "PrescriptionItem", on_delete=models.CASCADE, related_name="dispensing_logs"
    )
    dispensed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name="dispensing_actions",
    )
...
class Prescription(models.Model):
    ...
    invoice = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name="prescription_invoice",
    )
    ...
    def get_total_prescribed_price(self):  # Renamed for clarity
        ...
        for item in self.items.all():
            ...
            )  # item.quantity is prescribed quantity
        return total
    ...
    def can_be_dispensed(self):
        ...
        if self.requires_authorization:
            if not self.authorization_code:
                return (
                    False,
                    "Authorization code required for NHIA patient. Please obtain authorization code before dispensing.",
                )
    ...
    def get_dispensing_progress(self):
        ...
        not_dispensed = items.filter(
            is_dispensed=False, quantity_dispensed_so_far=0
        ).count()

        total_items = items.count()
        progress_percentage = (
            (fully_dispensed / total_items * 100) if total_items > 0 else 0
        )
...
class PrescriptionItem(models.Model):
    ...
    dosage = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Dosage instructions (e.g., 500mg, 10ml)",
    )
    frequency = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        help_text="How often to take (e.g., twice daily, once daily)",
    )
    duration = models.CharField(
        max_length=100,
        blank=True,
        null=True,
        help_text="How long to take (e.g., 7 days, 2 weeks)",
    )
    ...
    quantity = models.IntegerField(
        default=1, help_text="Quantity to dispense (managed at cart level)"
    )
    quantity_dispensed_so_far = models.IntegerField(
        default=0
    )  # Added back to fix IntegrityError
...
class PackItem(models.Model):
    ...
    item_type = models.CharField(
        max_length=20, choices=ITEM_TYPE_CHOICES, default="medication"
    )
    usage_instructions = models.TextField(
        blank=True, null=True, help_text="Instructions for using this item"
    )
    is_critical = models.BooleanField(
        default=False, help_text="Critical items cannot be substituted"
    )
    is_optional = models.BooleanField(
        default=False, help_text="Optional items can be omitted if unavailable"
    )
    order = models.IntegerField(
        default=0, help_text="Order of usage in procedure (0 for no specific order)"
    )
...
class DispensaryTransfer(models.Model):
    ...
    medication = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="dispensary_transfers"
    )
    from_active_store = models.ForeignKey(
        "ActiveStore",
        on_delete=models.CASCADE,
        related_name="outgoing_dispensary_transfers",
    )
    to_dispensary = models.ForeignKey(
        "Dispensary",
        on_delete=models.CASCADE,
        related_name="incoming_dispensary_transfers",
    )
    quantity = models.IntegerField()
    batch_number = models.CharField(max_length=50, blank=True, null=True)
    expiry_date = models.DateField(blank=True, null=True)
    ...
    requested_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="requested_dispensary_transfers",
    )
    approved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="approved_dispensary_transfers",
    )
    transferred_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="executed_dispensary_transfers",
    )
    notes = models.TextField(blank=True, null=True)
    requested_at = models.DateTimeField(auto_now_add=True)
    approved_at = models.DateTimeField(null=True, blank=True)
    ...
    def create_transfer(
        ...
        **kwargs,
        ...
        if not active_inventory:
            raise ValueError(
                f"Insufficient stock in {from_active_store.name}. Required: {quantity}"
            )

        # Check if medication is expired
        if active_inventory.expiry_date and active_inventory.expiry_date < date.today():
            raise ValueError(
                f"Cannot transfer expired medication. Batch {active_inventory.batch_number} expired on {active_inventory.expiry_date}"
            )
...
class PackOrder(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="pack_orders"
    )
    pack = models.ForeignKey("MedicalPack", on_delete=models.CASCADE)
    ordered_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="ordered_pack_orders",
    )
    ...
    processed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="processed_pack_orders",
    )
    processed_at = models.DateTimeField(null=True, blank=True)
    order_notes = models.TextField(
        blank=True, null=True, help_text="Additional notes about this pack order"
    )
    scheduled_date = models.DateTimeField(
        null=True, blank=True, help_text="When the pack is needed"
    )
    ...
    labor_record = models.ForeignKey(
        "labor.LaborRecord",
        on_delete=models.CASCADE,
        null=True,
        blank=True,
    ...
    processing_notes = models.TextField(
        blank=True, null=True, help_text="Notes about order processing"
    )
    approved_at = models.DateTimeField(null=True, blank=True)
    approved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
    ...
    def is_nhia_patient(self):
        """Check if the patient is an NHIA patient"""
        return (
            hasattr(self.patient, "patient_type")
            and self.patient.patient_type == "nhia"
        )
...
class MedicalPack(models.Model):
    ...
    surgery_type = models.CharField(
        max_length=50, choices=SURGERY_TYPE_CHOICES, blank=True, null=True
    )
    labor_type = models.CharField(
        max_length=50, choices=LABOR_TYPE_CHOICES, blank=True, null=True
    )
    ...
    requires_approval = models.BooleanField(
        default=False,
        help_text="Check if this pack requires approval before processing",
    )
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def can_be_ordered(self, patient=None, authorization_code=None):
        ...
        if patient:
            ...
            is_nhia = (
                hasattr(patient, "patient_type") and patient.patient_type == "nhia"
            )
            if is_nhia:
                ...
                if not authorization_code:
                    return (
                        False,
                        "NHIA authorization code required. Please obtain authorization from desk office before ordering this pack.",
                    )
                ...
                if hasattr(authorization_code, "is_valid"):
                    if not authorization_code.is_valid():
                        ...
                        return (
                            False,
                            f"Authorization code is {status}. Please obtain a valid authorization code.",
                        )
...
class MedicalPackItem(models.Model):
    ...
    pack = models.ForeignKey(
        MedicalPack, on_delete=models.CASCADE, related_name="items"
    )
    medication = models.ForeignKey(Medication, on_delete=models.CASCADE)
    quantity = models.IntegerField()
    item_type = models.CharField(
...
class InterDispensaryTransfer(models.Model):
    ...
    medication = models.ForeignKey(
        Medication, on_delete=models.CASCADE, related_name="inter_dispensary_transfers"
    )
    from_dispensary = models.ForeignKey(
        "Dispensary", on_delete=models.CASCADE, related_name="outgoing_inter_transfers"
    )
    to_dispensary = models.ForeignKey(
        "Dispensary", on_delete=models.CASCADE, related_name="incoming_inter_transfers"
    )
    quantity = models.IntegerField()
    batch_number = models.CharField(max_length=50, blank=True, null=True)
    expiry_date = models.DateField(blank=True, null=True)
    ...
    requested_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="requested_inter_transfers",
    )
    approved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="approved_inter_transfers",
    )
    transferred_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="executed_inter_transfers",
    )
    ...
    def check_availability(self):
        ...
        if inventory.stock_quantity < self.quantity:
            return (
                False,
                f"Insufficient stock. Available: {inventory.stock_quantity}, Required: {self.quantity}",
            )
    ...
    def create_transfer(
        ...
        **kwargs,
        ...
        # Check if sufficient stock exists
        can_transfer, message = cls.check_transfer_feasibility(
    ...
    def check_transfer_feasibility(medication, from_dispensary, quantity):
        ...
        if not inventory:
            return (
                False,
                f"No inventory found for {medication.name} in {from_dispensary.name}",
            )

        if inventory.stock_quantity < quantity:
            return (
                False,
                f"Insufficient stock. Available: {inventory.stock_quantity}, Required: {quantity}",
            )
