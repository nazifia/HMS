#!/usr/bin/env python
"""
File-based verification that the transfer button issues have been fixed
"""
...
def verify_view_function():
    ...
    try:
        ...
        if count == 1:
            ...
            # Check if inventory_items is in the function
            lines = content.split('\n')
            in_function = False
            function_lines = []
            
            for line in lines:
                if line.strip().startswith('def active_store_detail('):
                    in_function = True
                    function_lines.append(line)
                elif in_function:
                    if line.startswith('def ') or (line.startswith(' ') and not line.strip()):
                        # End of function or another function
                        if line.startswith('def '):
                            in_function = False
                        function_lines.append(line)
                    elif line.strip() and not line.startswith(' '):
                        # End of function (next top-level element)
                        in_function = False
                    else:
                        function_lines.append(line)
            
            function_content = '\n'.join(function_lines)
            if 'inventory_items' in function_content:
                print("   ✅ Function includes inventory_items - this is the correct version!")
                return True
            else:
                print("   ❌ Function does not include inventory_items - this might be incomplete!")
                return False
        elif count == 0:
            ...
            # Look for the complete version
            if 'inventory_items' in content:
                print("   ✅ At least one version includes inventory_items")
                return True
            else:
                print("   ❌ None of the versions include inventory_items")
                return False
...
def main():
    ...
    print("🧪 File-Based Verification of Transfer Button Fixes")
    print("=" * 55)
