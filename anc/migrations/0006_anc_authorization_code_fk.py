...
ANC stored the desk office's code as a CharField, so nothing checked it against
`nhia.AuthorizationCode`. Existing text is matched to a real code where one
exists and dropped where it does not — an unmatched string was never an
authorization.
"""
...
def link_existing_codes(apps, schema_editor):
    AncRecord = apps.get_model("anc", "AncRecord")
    AuthorizationCode = apps.get_model("nhia", "AuthorizationCode")

    for record in AncRecord.objects.exclude(
...
def unlink(apps, schema_editor):
    AncRecord = apps.get_model("anc", "AncRecord")
    for record in AncRecord.objects.filter(
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ("anc", "0005_ancclinicalnote_hospital_ancrecord_hospital"),
    ]
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.BooleanField(
                default=False,
                help_text="True if this NHIA patient ANC record requires desk office authorization",
            ),
        ),
        migrations.AddField(
            model_name="ancrecord",
            ...
            field=models.CharField(
                ...
                help_text="Status of authorization for this ANC record",
                max_length=20,
            ),
        ),
        migrations.AddField(
