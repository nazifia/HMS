...
class CustomUserAdmin(BaseUserAdmin):
    ...
    def get_role_badges(self, obj):
        ...
        roles = obj.roles.all()
        if not roles:
            return self.style_WARNING('No roles')
        ...
        for role in roles:
            ...
            badges.append(
                f'<span class="badge {badge_class}">{display_name}</span>'
            )
...
class RoleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('All Permissions (Read-Only)', {
            'fields': ('get_all_permissions_list',),
            'classes': ('collapse',)
        }),
    )
    ...
    def get_queryset(self, request):
        ...
        return super().get_queryset(request).prefetch_related(
            'permissions', 'customuser_roles', 'children'
        )
    ...
    def get_all_permissions_list(self, obj):
        ...
        perms_list = sorted([f"{p.content_type.app_label}.{p.codename}" for p in all_perms])
        if perms_list:
...
class CustomUserProfileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Employment Information', {
            'fields': ('employee_id', 'department', 'specialization', 'qualification')
        }),
        ('Contact Details', {
