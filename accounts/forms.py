    ...
    def clean(self):
        ...
        if username is not None and password:
            try:
                ...
                logger.error(
                    f"Unexpected error during form authentication: {e}", exc_info=True
                )
...
class CustomUserCreationForm(UserCreationForm):
    ...
    email = forms.EmailField(
        required=True
    )  # Make email required if it is for your logic
    ...
    class Meta(UserCreationForm.Meta):
        ...
        fields = (
            ...
            "first_name",
            "last_name",
        )  # 'phone_number' here maps to USERNAME_FIELD
    ...
    def save(self, commit=True):
        ...
        user.username = self.cleaned_data[
            "username"
        ]  # Ensure our specific username field is saved
...
class UserProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_instance:
            ...
            try:
                ...
                if profile_instance:
                    initial_data["contact_phone_number"] = getattr(
                        profile_instance, "phone_number", ""
                    )
                    initial_data["address"] = getattr(profile_instance, "address", "")
                    # profile_picture is handled by ImageField widget
                    initial_data["date_of_birth"] = getattr(
                        profile_instance, "date_of_birth", None
                    )
                    initial_data["department"] = getattr(
                        profile_instance, "department", None
                    )
                    ...
                    initial_data["specialization"] = getattr(
                        profile_instance, "specialization", ""
                    )
                    initial_data["qualification"] = getattr(
                        profile_instance, "qualification", ""
                    )
            except Exception as e:
    ...
    def clean_contact_phone_number(self):
        ...
        if phone:
            ...
            if hasattr(self, "profile_instance") and self.profile_instance:
                query = CustomUserProfile.objects.filter(phone_number=phone).exclude(
                    pk=self.profile_instance.pk
                )
                if query.exists():
                    raise ValidationError(
                        "This contact phone number is already in use on another profile."
                    )
            elif (
                not hasattr(self, "profile_instance")
                and CustomUserProfile.objects.filter(phone_number=phone).exists()
            ):
    ...
    def save(self, commit=True):
        ...
        if user_instance.pk:
            ...
            fresh_user = CustomUser.objects.only("phone_number").get(
                pk=user_instance.pk
            )
        ...
        user_instance.username = self.cleaned_data.get(
            "username", user_instance.username
        )
        user_instance.first_name = self.cleaned_data.get(
        ...
        # Admin/Staff fields
        if (
            "is_active_user" in self.cleaned_data
        ):  # Check if field is present (e.g. for staff editing staff)
        ...
        profile = (
            ...
            if user_instance and user_instance.pk
            else getattr(self, "profile_instance", None)
        )
...
class StaffCreationForm(
    UserCreationForm
):  # Base on UserCreationForm for password handling
    """Form for creating new staff members. Ensures username is set and at least one role is selected."""

    username = forms.CharField(
        max_length=150, required=True, label="Username (for display & records)"
    )
    # phone_number will be the login ID (USERNAME_FIELD)
    phone_number = forms.CharField(
        max_length=15, required=True, label="Phone Number (Login ID)"
    )
    ...
    profile_department_name = forms.CharField(
    ...
    employee_id_profile = forms.CharField(
        max_length=20, required=False, label="Employee ID (Profile)"
    )
    ...
    def clean_roles(self):
        ...
        if not roles or len(roles) == 0:
            raise ValidationError(
                "At least one module/role must be selected for staff."
            )
        return roles
...
class DepartmentForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            ),  # Assuming you initialize select2 via JS
        }
...
class PhoneNumberPasswordResetForm(PasswordResetForm):
    ...
    email = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                "class": "form-control",
                "placeholder": "Enter your login phone number",
            }
        ),
    )
    ...
    def clean_email(self):
        ...
        try:
            ...
            if (
                not user.email
            ):  # Check if the user has an email to send the reset link to
                raise ValidationError(
                    "This user account does not have an email address for password reset."
                )
...
class BulkUserActionForm(forms.Form):
    ...
    def clean(self):
        ...
        if action in ["assign_role", "remove_role"] and not role:
            raise ValidationError(
                "Role is required for role assignment/removal actions"
            )
...
class PermissionFilterForm(forms.Form):
    ...
    search = forms.CharField(
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Search permissions..."}
        ),
    )
