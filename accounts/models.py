...
class Role(models.Model):
    ...
    def get_all_permissions(self):
        ...
        if (
            ...
            and "permissions" in self._prefetched_objects_cache
        ):
        ...
        while current:
            ...
            if (
                hasattr(current, "_prefetched_objects_cache")
                and "permissions" in current._prefetched_objects_cache
            ):
...
class CustomUser(AbstractUser):
    ...
    def __str__(self):
        ...
        if (
            name and name != f"User #{self.pk}"
        ):  # Avoid returning the generic ID if a name exists
            return name
    ...
    def get_all_assigned_dispensaries(self):
        ...
        try:
            ...
            return [
                assignment.dispensary
                for assignment in assignments
                if assignment.dispensary.is_active
            ]
        except:
            return []
...
class CustomUserProfile(models.Model):
    ...
    is_active = models.BooleanField(
        default=True
    )  # Note: CustomUser also has is_active. Keep them synced if necessary.
...
class AuditLog(models.Model):
    ...
    target_user = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        blank=True,  # blank=True if target_user is not always applicable
        related_name="targeted_logs",
        verbose_name=_("affected user"),
    )
    action = models.CharField(
        max_length=25, choices=ACTION_CHOICES
    )  # Increased max_length for new choices
...
class EncryptedField(models.Field):
    ...
    def to_python(self, value):
        if (
            value is None or isinstance(value, str) and not value.startswith("gAAAAA")
    ...
    def decrypt(self, value):
        ...
        try:
            ...
            Exception
        ):  # Broad exception for decryption failure (e.g., invalid token, wrong key)
...
class EncryptedTextField(EncryptedField, models.TextField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        original_max_length = kwargs.pop(
            "max_length", None
        )  # Temporarily remove for TextField init
        super(EncryptedField, self).__init__(
            *args, **kwargs
        )  # Call models.TextField.__init__ via EncryptedField's MRO parent
        if (
            original_max_length is not None
        ):  # Put it back if it was there for EncryptedField logic
...
class UserActivity(models.Model):
    ...
    activity_level = models.CharField(
        max_length=10, choices=ACTIVITY_LEVELS, default="low"
    )
