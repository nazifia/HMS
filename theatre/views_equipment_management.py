...
def manage_surgery_type_equipment(request, surgery_type_id):
    ...
    available_equipment = SurgicalEquipment.objects.filter(
        is_available=True
    ).order_by('name')
    ...
    if request.method == 'POST':
        ...
        if action == 'add_equipment':
            ...
            try:
                ...
                messages.success(request, f'Added {equipment.name} to required equipment.')
            ...
            if pack_surgery_type:
                    ...
                    for item in suggested_equipment:
                        ...
                        equipment, created = SurgicalEquipment.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                'description': item['medication'].description or f"Equipment for {surgery_type.name}",
                        ...
                        surgery_type_equip, created = SurgeryTypeEquipment.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                'notes': item['usage_instructions'] or f"From {item['pack_name']}",
                            }
                        )
                ...
                messages.warning(request, 'No matching medical packs found for this surgery type.')
        
        return redirect('theatre:manage_surgery_type_equipment', surgery_type_id=surgery_type_id)
    
    # Calculate statistics
    mandatory_count = sum(1 for e in current_equipment if e.is_mandatory)
    available_count = sum(
        1 for e in current_equipment
