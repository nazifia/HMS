...
class SurgeryType(models.Model):
    ...
    average_duration = models.DurationField(
        help_text="Expected duration of surgery (HH:MM:SS)"
    )
    preparation_time = models.DurationField(
        help_text="Time needed for preparation before surgery"
    )
    recovery_time = models.DurationField(
        help_text="Expected recovery time after surgery"
    )
    risk_level = models.CharField(max_length=20, choices=RISK_LEVELS, default="medium")
    instructions = models.TextField(
        blank=True, null=True, help_text="Special instructions for this surgery type"
    )
...
class Surgery(models.Model):
    ...
    surgery_type = models.ForeignKey(
        SurgeryType, on_delete=models.PROTECT, related_name="surgeries"
    )
    theatre = models.ForeignKey(
        OperationTheatre, on_delete=models.SET_NULL, null=True, related_name="surgeries"
    )
    ...
    def can_be_performed(self):
        ...
        if self.requires_authorization:
            if not self.authorization_code:
                return (
                    False,
                    "Desk office authorization required for NHIA patient surgery. Please obtain authorization code before proceeding.",
                )
...
class SurgicalTeam(models.Model):
    ...
    surgery = models.ForeignKey(
        Surgery, on_delete=models.CASCADE, related_name="team_members"
    )
    staff = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="surgical_assignments"
    )
...
class SurgicalEquipment(models.Model):
    ...
    calibration_frequency = models.DurationField(
        blank=True, null=True, help_text="e.g., '365 00:00:00' for annual calibration"
    )
    ...
    def __str__(self):
        ...
        equipment_type_display = equipment_type_display_map.get(
            self.equipment_type, self.equipment_type
        )
...
class EquipmentUsage(models.Model):
    ...
    surgery = models.ForeignKey(
        Surgery, on_delete=models.CASCADE, related_name="equipment_used"
    )
    equipment = models.ForeignKey(
        SurgicalEquipment, on_delete=models.CASCADE, related_name="usage_records"
    )
...
class SurgerySchedule(models.Model):
    ...
    surgery = models.OneToOneField(
        Surgery, on_delete=models.CASCADE, related_name="schedule"
    )
...
class PostOperativeNote(models.Model):
    ...
    surgery = models.ForeignKey(
        Surgery, on_delete=models.CASCADE, related_name="post_op_notes"
    )
...
class SurgeryTypeEquipment(models.Model):
    ...
    surgery_type = models.ForeignKey(
        SurgeryType, on_delete=models.CASCADE, related_name="required_equipment"
    )
    equipment = models.ForeignKey(
        SurgicalEquipment,
        on_delete=models.CASCADE,
        related_name="surgery_types_required",
    )
    quantity_required = models.PositiveIntegerField(
        default=1, help_text="Quantity of this equipment needed for the surgery"
    )
    is_mandatory = models.BooleanField(
        default=True,
        help_text="If true, this equipment must be available for the surgery to proceed",
    )
    notes = models.TextField(
        blank=True, null=True, help_text="Special instructions for using this equipment"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class SurgeryLog(models.Model):
    ...
    event_type = models.CharField(
        max_length=100
    )  # e.g., 'Surgery Started', 'Complication Noted', 'Equipment Used'
...
class EquipmentMaintenanceLog(models.Model):
    ...
    equipment = models.ForeignKey(
        SurgicalEquipment, on_delete=models.CASCADE, related_name="maintenance_logs"
    )
    maintenance_type = models.CharField(
        max_length=20, choices=MAINTENANCE_TYPE_CHOICES, default="maintenance"
    )

    # Dates
    scheduled_date = models.DateTimeField(help_text="When maintenance is scheduled")
    completed_date = models.DateTimeField(
        blank=True, null=True, help_text="When maintenance was actually completed"
    )
    next_due_date = models.DateTimeField(
        blank=True, null=True, help_text="When next maintenance is due"
    )

    # Details
    description = models.TextField(
        help_text="Description of work performed or to be performed"
    )
    findings = models.TextField(
        blank=True, null=True, help_text="Findings during maintenance"
    )
    parts_replaced = models.TextField(
        blank=True, null=True, help_text="Parts that were replaced"
    )
    ...
    external_provider = models.CharField(
        max_length=200, blank=True, null=True, help_text="External service company"
    )
    external_technician = models.CharField(
        max_length=200, blank=True, null=True, help_text="Technician name"
    )
