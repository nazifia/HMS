...
class SurgeryForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "expected_duration": forms.TextInput(attrs={"placeholder": "HH:MM:SS"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set theatre queryset - include available theatres plus currently selected (if editing)
        if "theatre" in self.fields:
            theatre_qs = OperationTheatre.objects.filter(is_available=True)
            # If editing an existing surgery, include the currently selected theatre even if not available
            if self.instance and self.instance.pk and self.instance.theatre:
                theatre_qs = theatre_qs | OperationTheatre.objects.filter(
                    pk=self.instance.theatre.pk
                )
        ...
        if "primary_surgeon" in self.fields:
            ...
            if raw_value:
                ...
                self.instance and self.instance.pk and self.instance.primary_surgeon_id
            ):
            ...
            # ALWAYS add selected surgeon to queryset - even if they don't match the filter
        ...
        if "anesthetist" in self.fields:
            ...
            # ALWAYS add selected anesthetist to queryset - even if they don't match the filter
            # This prevents "user does not exist" errors when editing
    ...
    def clean(self):
        ...
        if primary_surgeon:
            try:
                user = CustomUser.objects.get(pk=primary_surgeon.pk)
                # If user is inactive, warn but allow saving
                if not user.is_active:
                    self._warnings.append(
                        f"Warning: Selected surgeon {user.get_full_name()} is currently inactive."
                    )
            except CustomUser.DoesNotExist:
                cleaned_data["primary_surgeon"] = None
                self.add_error(
                    "primary_surgeon",
                    "Selected surgeon no longer exists. Please select a different surgeon.",
                )

        if anesthetist:
            try:
                user = CustomUser.objects.get(pk=anesthetist.pk)
                # If user is inactive, warn but allow saving
                if not user.is_active:
                    self._warnings.append(
                        f"Warning: Selected anesthetist {user.get_full_name()} is currently inactive."
                    )
            except CustomUser.DoesNotExist:
                cleaned_data["anesthetist"] = None
                self.add_error(
                    "anesthetist",
                    "Selected anesthetist no longer exists. Please select a different anesthetist.",
                )
        ...
        if theatre and scheduled_date and expected_duration:
            ...
            if conflicts:
                if skip_conflict:
                    ...
                    for conflict in conflicts:
                        self.add_error(
                            "scheduled_date", f"Scheduling conflict: {conflict}"
                        )
...
class SurgicalTeamMultipleForm(forms.Form):
    """Form for adding multiple team members at once."""
    ...
    usage_notes = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 2}), required=False, label="Usage Notes"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if surgery_id:
            ...
            self.fields["surgery"].disabled = True
...
class EquipmentUsageForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Include all equipment in queryset (not just available)
        if "equipment" in self.fields:
            self.fields[
                "equipment"
            ].queryset = SurgicalEquipment.objects.all().order_by("name")
...
class SurgeryScheduleForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            "start_time",
            "end_time",
            "pre_op_preparation_start",
            "post_op_recovery_end",
            "status",
            "delay_reason",
        ]
        widgets = {
...
class PreOperativeChecklistForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            "patient_identified",
            "site_marked",
            "anesthesia_safety_check_completed",
            "surgical_safety_checklist_completed",
            "consent_confirmed",
            "allergies_reviewed",
            "imaging_available",
...
class EquipmentMaintenanceLogForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "findings": forms.Textarea(
                attrs={"rows": 3, "placeholder": "Any findings during maintenance"}
            ),
            "parts_replaced": forms.Textarea(
                attrs={"rows": 2, "placeholder": "List any parts that were replaced"}
            ),
            "external_provider": forms.TextInput(
                attrs={"placeholder": "External service company name"}
            ),
            "external_technician": forms.TextInput(
                attrs={"placeholder": "Technician name"}
            ),
            "certificate_number": forms.TextInput(
                attrs={"placeholder": "Calibration certificate number"}
            ),
        }
    ...
    def clean(self):
        ...
        if next_due_date and completed_date and next_due_date <= completed_date:
            self.add_error(
                "next_due_date", "Next due date should be after completed date."
            )
