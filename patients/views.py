...
def patient_list(request):
    ...
    if request.GET:
        ...
        if retainership_number:
            patients = patients.filter(
                retainership_info__retainership_reg_number__icontains=retainership_number
            )
        if date_from:
            try:
    ...
    cache_key = (
        f"patient_list_data_{hash(request.GET.urlencode())}"
        if request.GET
        else "patient_list_data_all"
    )
...
def patient_detail(request, patient_id):
    ...
    recent_consultations = (
        ...
        .select_related("doctor", "consulting_room")
    ...
    physiotherapy_requests = (
        PhysiotherapyRequest.objects.filter(patient=patient)
        .select_related("referring_doctor")
    ...
    if hasattr(patient, "wallet_memberships"):
        retainership_wallet = (
            patient.wallet_memberships.select_related("wallet")
            .filter(wallet__wallet_type="retainership")
            .first()
        )
...
def edit_patient(request, patient_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            patient = form.save()
            messages.success(
                request, f"Patient {patient.get_full_name()} updated successfully."
            )
            return redirect("patients:detail", patient_id=patient.id)
    else:
...
def edit_medical_history(request, history_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "patients:medical_history", patient_id=medical_history.patient.id
            )
    else:
    ...
    if request.method == "POST":
        ...
        if vitals_form.is_valid():
            ...
            if not vital.recorded_by and request.user.is_authenticated:
                if (
                    hasattr(request.user, "get_full_name")
    ...
    if not vitals:
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FROM patients_vitals WHERE patient_id = %s",
                    [patient_id],
                )
            ...
            if total_count > 0:
                messages.warning(
                    request,
                    "Some vital records could not be displayed due to data issues.",
                )
        except Exception:
            pass
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if apply_to_outstanding and total_outstanding > 0:
                ...
                messages.success(
                    request,
                    f"Successfully added ₦{amount} to {patient.get_full_name()}'s wallet. ₦{amount_applied} automatically applied to outstanding charges.",
                )
            else:
                messages.success(
                    request,
                    f"Successfully added ₦{amount} to {patient.get_full_name()}'s wallet.",
                )
...
def wallet_transactions(request, patient_id):
    ...
    if created:
        messages.info(
            request,
            f"A wallet has been automatically created for {patient.get_full_name()}.",
        )
...
def wallet_withdrawal(request, patient_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Successfully withdrew ₦{amount} from {patient.get_full_name()}'s wallet.",
            )
            return redirect("patients:wallet_dashboard", patient_id=patient.id)
    else:
...
def wallet_transfer(request, patient_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            recipient_wallet, created = PatientWallet.objects.get_or_create(
            ...
            try:
                ...
                messages.success(
                    request,
                    f"Successfully transferred ₦{amount} from {patient.get_full_name()}'s wallet to {recipient_patient.get_full_name()}'s wallet.",
                )
                return redirect("patients:wallet_dashboard", patient_id=patient.id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            wallet.credit(
                ...
                + (f" (Invoice: {reference_invoice})" if reference_invoice else ""),
            ...
            messages.success(
                request,
                f"Successfully refunded ₦{amount} to {patient.get_full_name()}'s wallet.",
            )
            return redirect("patients:wallet_dashboard", patient_id=patient.id)
    else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if adjustment_type == "credit":
                ...
                messages.success(
                    request,
                    f"Successfully credited ₦{amount} to {patient.get_full_name()}'s wallet.",
                )
            else:  # debit
                # Debit the wallet
                wallet.debit(
                ...
                messages.success(
                    request,
                    f"Successfully debited ₦{amount} from {patient.get_full_name()}'s wallet.",
                )
    ...
    if request.method == "POST":
        try:
            ...
            settlement_result = wallet.settle_outstanding_balance(
                description="Outstanding balance settlement", user=request.user
            )
...
def wallet_payment(request, patient_id):
    ...
    if request.method == "POST":
        try:
            ...
            settlement_result = wallet.settle_outstanding_balance(
                description="Wallet payment for outstanding amounts", user=request.user
            )
...
def wallet_net_impact_global(request):
    ...
    wallets = PatientWallet.objects.select_related("patient").filter(
    ...
    if request.headers.get("HX-Request"):
        return render(
            request, "patients/partials/wallet_net_impact_table.html", context
        )
...
def wallet_net_impact(request, patient_id):
    ...
    net_impact_without_update = wallet.get_total_wallet_impact_with_admissions(
...
def register_nhia_patient(request, patient_id):
    ...
    if hasattr(patient, "nhia_info"):
        messages.warning(
            request, f"{patient.get_full_name()} is already registered for NHIA."
        )
        return redirect("patients:detail", patient_id=patient.id)
    ...
    if request.method == "POST":
        ...
        nhia_patient = NHIAPatient.objects.create(
            patient=patient, nhia_reg_number=generate_nhia_reg_number(), is_active=True
        )

        messages.success(
            request,
            f"{patient.get_full_name()} has been successfully registered for NHIA with registration number {nhia_patient.nhia_reg_number}.",
        )
        return redirect("patients:detail", patient_id=patient.id)
...
def edit_nhia_patient(request, patient_id):
    ...
    if not hasattr(patient, "nhia_info"):
        messages.error(
            request, f"{patient.get_full_name()} is not registered for NHIA."
        )
        return redirect("patients:detail", patient_id=patient.id)
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"{patient.get_full_name()} NHIA registration has been {status}."
        )
        return redirect("patients:detail", patient_id=patient.id)
...
def register_retainership_patient(request, patient_id):
    ...
    if hasattr(patient, "retainership_info"):
        messages.warning(
            request,
            f"{patient.get_full_name()} is already registered for retainership.",
        )
        return redirect("patients:detail", patient_id=patient.id)
    ...
    if request.method == "POST":
        ...
        while True:
            ...
            if not RetainershipPatient.objects.filter(
        ...
        messages.success(
            request,
            f"{patient.get_full_name()} has been successfully registered for retainership with registration number {retainership_patient.retainership_reg_number}.",
        )
        return redirect("patients:detail", patient_id=patient.id)
...
def add_vaccination(request, patient_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                f"Vaccination record added successfully for {patient.get_full_name()}.",
            )
            return redirect("patients:detail", patient_id=patient.id)
...
def edit_retainership_patient(request, patient_id):
    ...
    if not hasattr(patient, "retainership_info"):
        messages.error(
            request, f"{patient.get_full_name()} is not registered for retainership."
        )
        return redirect("patients:detail", patient_id=patient.id)
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"{patient.get_full_name()} retainership registration has been {status}.",
        )
        return redirect("patients:detail", patient_id=patient.id)
...
def patient_dashboard(request, patient_id):
    ...
    recent_appointments = Appointment.objects.filter(
        patient=patient, appointment_date__gte=six_months_ago
    ...
    recent_consultations = (
        Consultation.objects.select_related("doctor", "consulting_room")
    ...
    recent_prescriptions = (
        Prescription.objects.select_related("doctor")
        .filter(patient=patient)
        .order_by("-prescription_date")[:5]
    )
    ...
    recent_lab_tests = (
        TestRequest.objects.select_related("test", "requesting_doctor")
        .filter(patient=patient)
        .order_by("-request_date")[:5]
    )

    # Get recent radiology orders
    recent_radiology_orders = (
        RadiologyOrder.objects.select_related("radiology_test", "requesting_doctor")
        .filter(patient=patient)
...
def clear_patient_context(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse(
            {"success": True, "message": "Patient context cleared successfully"}
        )
...
def edit_physiotherapy_request(request, request_id):
    ...
    if request.method == "POST":
        form = PhysiotherapyRequestForm(
            request.POST, instance=physiotherapy_request, user=request.user
        )
        if form.is_valid():
            form.save()
            messages.success(request, "Physiotherapy request updated successfully.")
            return redirect(
                "patients:detail", patient_id=physiotherapy_request.patient.id
            )
    else:
        form = PhysiotherapyRequestForm(
            instance=physiotherapy_request, user=request.user
        )
        ...
        try:
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    is_nhia_patient = (
                        hasattr(patient, "nhia_info")
                        and patient.nhia_info
                        and patient.nhia_info.is_active
                    )
