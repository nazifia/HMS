...
class RadiologyCategory(TenantModel):
    ...
    invoice = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name="radiology_category_invoice",
    )
...
class RadiologyTest(TenantModel):
    ...
    category = models.ForeignKey(
        RadiologyCategory, on_delete=models.CASCADE, related_name="tests"
    )
    description = models.TextField(blank=True, null=True)
    ...
    invoice = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name="radiology_test_invoice",
    )
...
class RadiologyOrder(TenantModel):
    ...
    patient = models.ForeignKey(
        Patient, on_delete=models.CASCADE, related_name="radiology_orders"
    )
    test = models.ForeignKey(
        RadiologyTest, on_delete=models.CASCADE, related_name="orders"
    )
    referring_doctor = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="radiology_referrals",
    )
    technician = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="radiology_performed",
    )
    ...
    def can_add_result(self):
        ...
        if self.requires_authorization:
            if not self.authorization_code:
                return (
                    False,
                    "Desk office authorization required for NHIA patient from non-NHIA unit. Please obtain authorization code before adding results.",
                )
            elif not self.authorization_code.is_valid():
                return (
                    False,
        ...
        if self.status not in ["completed", "payment_confirmed", "scheduled"]:
            return (
                False,
                f"Order must be completed before adding results. Current status: {self.get_status_display()}",
            )
...
class RadiologyResult(TenantModel):
    ...
    order = models.OneToOneField(
        RadiologyOrder, on_delete=models.CASCADE, related_name="result"
    )
    performed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="performed_radiology",
    )
    ...
    invoice = models.OneToOneField(
        ...
        null=True,
        blank=True,
        related_name="radiology_result_invoice",
    )
    ...
    contrast_used = models.CharField(
        max_length=20, choices=CONTRAST_CHOICES, default="none"
    )
    contrast_amount = models.DecimalField(
    ...
    study_status = models.CharField(
        max_length=20, choices=STUDY_STATUS_CHOICES, default="complete"
    )
    notes = models.TextField(blank=True, null=True)
    result_status = models.CharField(
        max_length=20, choices=RESULT_STATUS_CHOICES, default="draft"
    )
