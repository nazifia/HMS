...
def index(request):
    ...
    if user_department:
        ...
        context["daily_trend"] = get_daily_trend_data(
            RadiologyOrder, days=7, date_field="order_date"
        )
        context["status_distribution"] = get_status_distribution(
            RadiologyOrder, status_field="status"
        )
        context["completion_rate"] = calculate_completion_rate(
            RadiologyOrder, status_field="status", completed_status="completed"
        )
        context["urgent_items"] = get_urgent_items(
            RadiologyOrder, priority_field="priority"
        )
        ...
        context = {
            ...
            "recent_records": RadiologyOrder.objects.select_related(
                "patient", "test"
    ...
    today_completed = RadiologyOrder.objects.filter(
        order_date__date=today, status="completed"
    ).count()

    # This week's statistics
    week_orders = RadiologyOrder.objects.filter(
        order_date__date__gte=week_start
    ).count()
    week_completed = RadiologyOrder.objects.filter(
        order_date__date__gte=week_start, status="completed"
    ).count()

    # This month's statistics
    month_orders = RadiologyOrder.objects.filter(
        order_date__date__gte=month_start
    ).count()
    month_revenue = (
        RadiologyOrder.objects.filter(
            order_date__date__gte=month_start, invoice__isnull=False
        ).aggregate(total=Sum("invoice__total_amount"))["total"]
        or 0
    )
    ...
    urgent_orders = (
        ...
        .select_related("patient", "test", "referring_doctor")
        .order_by("-priority", "order_date")[:10]
    )

    # Get emergency orders count
    emergency_orders = RadiologyOrder.objects.filter(
        status__in=["pending", "scheduled"], priority="emergency"
    ).count()
    ...
    avg_reporting_time = (
        ...
        .annotate(
            reporting_time=ExpressionWrapper(
                F("completed_date") - F("created_at"), output_field=DurationField()
            )
        )
    ...
    modality_data = (
        RadiologyOrder.objects.values("test__name")
    ...
    results_needing_verification = (
        RadiologyResult.objects.filter(verified_by__isnull=True)
        .select_related("order__patient")
        .count()
    )

    # Get completed orders without results (for action alert)
    orders_needing_results = (
        RadiologyOrder.objects.filter(status="completed", result__isnull=True)
        .select_related("patient", "test")
        .order_by("-order_date")[:10]
    )

    # Build query with filters first
    recent_orders_query = RadiologyOrder.objects.select_related(
        "patient", "test", "referring_doctor", "result"
    )
    ...
    if patient_id_filter:
        recent_orders_query = recent_orders_query.filter(
            patient__patient_id=patient_id_filter
        )
        ...
        if form.is_valid():
            ...
            if authorization_code_id:
                try:
                    ...
                    if not authorization_code.is_valid():
                        messages.error(
                            request, "The provided authorization code is not valid."
                        )
                        return (
                            redirect("radiology:order_radiology", patient_id=patient_id)
                            if patient_id
                            else redirect("radiology:order_radiology")
                        )
                    ...
                    messages.error(
                        request, "The provided authorization code does not exist."
                    )
                    return (
                        redirect("radiology:order_radiology", patient_id=patient_id)
                        if patient_id
            ...
            with transaction.atomic():
                ...
                order.authorization_code = (
                    authorization_code  # Set authorization code if provided
                )
                order.save()
                ...
                due_date = order.order_date.date() + timedelta(
                    days=7
                ...
                radiology_service_category, _ = ServiceCategory.objects.get_or_create(
                ...
                service, _ = Service.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        or f"Radiology test: {order.test.name}",
                    },
                )
                if (
                    service.price != order.test.price
                ):  # Update service price if it differs from test price
                    service.price = order.test.price
                    service.save()
                ...
                InvoiceItem.objects.create(
                    ...
                    tax_amount=(Decimal(order.test.price) * service.tax_percentage)
                    / Decimal("100"),
                    total_amount=Decimal(order.test.price)
                    + (
                        (Decimal(order.test.price) * service.tax_percentage)
                        / Decimal("100")
                    ),
                )
            ...
            messages.success(
                request,
                f"Radiology order created successfully. Invoice #{invoice.invoice_number} generated and is {'paid via authorization code' if authorization_code else 'pending payment'}.",
            )
...
def edit_order(request, order_id):
    ...
    return render(
        request,
        "radiology/order_form.html",
        {"form": form, "order": order, "selected_patient": order.patient},
    )
...
def add_result(request, order_id):
    ...
    return render(
        ...
        {"form": form, "order": order, "result": result},
    )
...
def radiology_sales_report(request):
    ...
    monthly_revenue = (
        RadiologyOrder.objects.filter(
            order_date__date__gte=month_start, order_date__date__lte=month_end
        ).aggregate(total=Sum("invoice__total_amount"))["total"]
        or 0
    )
...
def patient_radiology_results(request, patient_id):
    ...
    orders_with_results = (
        RadiologyOrder.objects.filter(patient=patient, status="completed")
        .select_related("test", "result")
        .order_by("-order_date")
    )
    results = RadiologyResult.objects.filter(
        order__in=orders_with_results
...
def result_detail(request, result_id):
    """View details of a specific radiology result."""
    result = get_object_or_404(
        RadiologyResult.objects.select_related(
            "order", "order__patient", "performed_by"
        ),
        pk=result_id,
    )
    context = {
        "result": result,
        "title": f"Radiology Result - {result.order.test.name}",
    }
    return render(request, "radiology/result_detail.html", context)
...
def radiology_statistics_report(request):
    ...
    # Orders by category
    category_stats = (
        radiology_orders.values("test__category__name", "test__category__id")
        .annotate(
    ...
    # Top tests by volume
    top_tests = (
        radiology_orders.values("test__name", "test__id")
        .annotate(
            total_orders=Count("id"),
            total_revenue=Sum("test__price"),
            unique_patients=Count("patient", distinct=True),
    ...
    # Status distribution
    status_stats = (
        radiology_orders.values("status").annotate(count=Count("id")).order_by("-count")
    )

    # Priority distribution
    priority_stats = (
        radiology_orders.values("priority")
        .annotate(count=Count("id"))
        .order_by("-count")
    )
    ...
    daily_stats = (
        radiology_orders.extra(select={"day": "DATE(order_date)"})
        .values("day")
        .annotate(daily_orders=Count("id"), daily_revenue=Sum("test__price"))
        .order_by("day")
    )

    # Top referring doctors
    top_doctors = (
        radiology_orders.values(
    ...
    # Top technicians
    top_technicians = (
        radiology_orders.filter(technician__isnull=False)
        .values("technician__first_name", "technician__last_name", "technician__id")
        .annotate(
            total_orders=Count("id"),
            total_revenue=Sum("test__price"),
            unique_patients=Count("patient", distinct=True),
    ...
    completed_with_times = radiology_orders.filter(
        status="completed", completed_date__isnull=False
    )
